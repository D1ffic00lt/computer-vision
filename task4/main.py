...
def check_answer(real_answer, user_answer):
    if IoU(real_answer, user_answer) < DETECTION_THRESHOLD:
        return False
...
def main():
    ...
    for i, row in enumerate(data.itertuples()):
        row_id, image_filename, x, y, w, h = row
        real_bbox = (x, y, w, h)

        image = cv2.imread(image_filename)

        user_answer = submission.detect_traffic_light(image, models)

        if check_answer(real_bbox, user_answer):
            correct += 1
